...
RESULT_CARTOVIEW=`su - postgres -c "psql -l | grep -w cartoview | wc -l"`
echo "Check default db exists"
if [[ ! ${RESULT_CARTOVIEW} == '1' ]]; then
  echo "Create default db cartoview"
  su - postgres -c "createdb -O ${POSTGRES_USER} -T template_postgis cartoview"
else
  echo "cartoview db already exists"
fi
RESULT_DATASTORE=`su - postgres -c "psql -l | grep -w cartoview_datastore | wc -l"`
echo "Check default db exists"
if [[ ! ${RESULT_DATASTORE} == '1' ]]; then
  echo "Create default db cartoview_datastore"
  su - postgres -c "createdb -O ${POSTGRES_USER} -T template_postgis cartoview_datastore"
else
  echo "cartoview_datastore db already exists"
fi
